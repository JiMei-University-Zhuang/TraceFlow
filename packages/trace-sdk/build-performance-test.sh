...
echo -e "${YELLOW}开始构建TraceFlow SDK整合测试...${NC}"
...
# 执行构建
echo -e "${YELLOW}构建完整SDK（包含错误、事件和性能监控）...${NC}"
npm run build:sdk
...
(function (global, factory) {
    ...
    class MockSDK {
        ...
        captureEvent(eventName, eventData) {
            console.log('[TraceFlow] 手动上报事件:', eventName, eventData);
            return this;
        }
        
        // 手动上报性能指标
        trackPerformance(metricName, value, unit, metadata) {
            console.log('[TraceFlow] 手动上报性能指标:', metricName, value, unit, metadata);
            return this;
        }
        
        // 立即上报所有事件
        flush() {
            console.log('[TraceFlow] 立即上报所有事件');
            return this;
        }
    }
    ...
    exports.init = function(options) {
        return new MockSDK(options);
    };
    
    // 导出类型
    exports.TraceEventType = TraceEventType;
...
# 创建综合测试页面
echo -e "${YELLOW}创建综合测试页面...${NC}"
cat > test-all.html << 'EOL'
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  ...
  fs.readFile(filePath, (error, content) => {
    if (error) {
      if (error.code === 'ENOENT') {
        // 文件不存在，尝试返回测试页面
        fs.readFile('./test-all.html', (err, data) => {
          if (err) {
            res.writeHead(404);
...
echo -e "${GREEN}构建和设置完成！${NC}"
echo -e "${YELLOW}运行服务器中...${NC}"
